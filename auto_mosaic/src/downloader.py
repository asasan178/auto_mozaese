...
from auto_mosaic.src.utils import get_models_dir, get_app_data_dir, logger
...
class ModelDownloader:
    ...
    def __init__(self):
        ...
        # Download log file (logs directory only)
        self.download_log = get_app_data_dir() / "logs" / "downloads.log"
    ...
    def _log_download(self, message: str):
        ...
        with self._log_lock:
            ...
            self.download_log.parent.mkdir(parents=True, exist_ok=True)
    ...
    def find_model_files_in_search_paths(self, model_name: str) -> Dict[str, Optional[Path]]:
        """
        柔軟なモデルファイル検索 - 複数の場所から検索
        
        Args:
            model_name: モデル名 (例: "anime_nsfw_v4")
            
        Returns:
            Dict[str, Optional[Path]]: 各モデルファイルのパス {"all": Path, "penis": Path, ...}
        """
        import sys
        import os
        from pathlib import Path
        ...
        # 1. 標準のmodels/anime_nsfw_v4ディレクトリ
        standard_dir = self.models_dir / model_info["extract_dir"]
        search_dirs.append(("Standard location", standard_dir))
        
        # 2. modelsフォルダ直下
        models_root = self.models_dir
        search_dirs.append(("Models root", models_root))
        
        # 3. exe環境の場合：実行ファイルと同一フォルダ
        if getattr(sys, 'frozen', False):
            if hasattr(sys, '_MEIPASS'):
                # PyInstallerワンファイル実行の場合
                exe_dir = Path(sys.executable).parent
            else:
                # PyInstallerワンディレクトリ実行の場合
                exe_dir = Path(sys.executable).parent
            
            search_dirs.append(("Executable directory", exe_dir))
            search_dirs.append(("Executable/models", exe_dir / "models"))
            search_dirs.append(("Executable/anime_nsfw_v4", exe_dir / "anime_nsfw_v4"))
        
        # 4. 開発環境の場合：プロジェクトルート直下も検索
        else:
            project_root = Path(__file__).parent.parent.parent
            search_dirs.append(("Project root", project_root))
            search_dirs.append(("Project root/anime_nsfw_v4", project_root / "anime_nsfw_v4"))
        
        # 各検索ディレクトリでモデルファイルを探索
        found_models = {}
        main_models = model_info["main_models"]
        
        logger.info(f"🔍 Searching for {model_name} models in multiple locations...")
        
        for location_name, search_dir in search_dirs:
            if not search_dir.exists():
                logger.debug(f"  ❌ {location_name}: {search_dir} (not found)")
                continue
                
            logger.debug(f"  🔍 {location_name}: {search_dir}")
            
            # このディレクトリ内でモデルファイルを検索
            for model_key, filename in main_models.items():
                if model_key in found_models:
                    continue  # 既に見つかっているファイルはスキップ
                
                # 直接ファイルを探す
                model_path = search_dir / filename
                if model_path.exists() and model_path.stat().st_size > 0:
                    found_models[model_key] = model_path
                    logger.info(f"    ✅ Found {model_key}: {model_path}")
                    continue
                
                # サブディレクトリ内も検索（最大2階層）
                for subdir in search_dir.iterdir():
                    if subdir.is_dir():
                        sub_model_path = subdir / filename
                        if sub_model_path.exists() and sub_model_path.stat().st_size > 0:
                            found_models[model_key] = sub_model_path
                            logger.info(f"    ✅ Found {model_key}: {sub_model_path}")
                            break
                            
                        # さらに1階層下も検索（anime_nsfw_v4フォルダなど）
                        for sub_subdir in subdir.iterdir():
                            if sub_subdir.is_dir():
                                sub_sub_model_path = sub_subdir / filename
                                if sub_sub_model_path.exists() and sub_sub_model_path.stat().st_size > 0:
                                    found_models[model_key] = sub_sub_model_path
                                    logger.info(f"    ✅ Found {model_key}: {sub_sub_model_path}")
                                    break
        
        # 検索結果をログ出力
        if found_models:
            logger.info(f"✅ Found {len(found_models)}/{len(main_models)} model files for {model_name}")
            for model_key, path in found_models.items():
                logger.info(f"  - {model_key}: {path}")
        else:
            logger.warning(f"❌ No model files found for {model_name}")
            logger.info("📂 Searched locations:")
            for location_name, search_dir in search_dirs:
                status = "✅" if search_dir.exists() else "❌"
                logger.info(f"  {status} {location_name}: {search_dir}")
        
        return found_models

    def find_single_model_in_search_paths(self, model_name: str) -> Optional[Path]:
        """
        単一ファイルモデルの柔軟な検索 - 複数の場所から検索
        
        Args:
            model_name: モデル名 (例: "sam_vit_b")
            
        Returns:
            Optional[Path]: 見つかったモデルファイルのパス
        """
        import sys
        import os
        from pathlib import Path
        ...
        # 1. 標準のmodelsディレクトリ
        search_dirs.append(("Standard models", self.models_dir))
        
        # 2. exe環境の場合：実行ファイルと同一フォルダ
        if getattr(sys, 'frozen', False):
            if hasattr(sys, '_MEIPASS'):
                # PyInstallerワンファイル実行の場合
                exe_dir = Path(sys.executable).parent
        ...
        # 3. 開発環境の場合：プロジェクトルート直下も検索
        else:
            project_root = Path(__file__).parent.parent.parent
            search_dirs.append(("Project root", project_root))
            search_dirs.append(("Project root/models", project_root / "models"))
        
        logger.debug(f"🔍 Searching for {model_name} ({filename}) in multiple locations...")
        ...
        for location_name, search_dir in search_dirs:
            ...
            for subdir in search_dir.iterdir():
                if subdir.is_dir():
                    sub_model_path = subdir / filename
                    if sub_model_path.exists() and sub_model_path.stat().st_size > 0:
                        logger.info(f"✅ Found {model_name}: {sub_model_path}")
                    ...
                    for sub_subdir in subdir.iterdir():
                        if sub_subdir.is_dir():
                            sub_sub_model_path = sub_subdir / filename
                            if sub_sub_model_path.exists() and sub_sub_model_path.stat().st_size > 0:
                                logger.info(f"✅ Found {model_name}: {sub_sub_model_path}")
                                return sub_sub_model_path
        
        logger.warning(f"❌ Model file {filename} not found for {model_name}")
        logger.info("📂 Searched locations:")
        for location_name, search_dir in search_dirs:
            status = "✅" if search_dir.exists() else "❌"
            logger.info(f"  {status} {location_name}: {search_dir}")
    ...
    def is_model_available(self, model_name: str) -> bool:
        ...
        if model_info["type"] == "zip":
            # 柔軟検索を使用してモデルの存在確認
            found_models = self.find_model_files_in_search_paths(model_name)
            # 少なくとも "all" モデルが見つかればOK
            return "all" in found_models and found_models["all"] is not None
            ...
            # Single file model - 柔軟検索を使用
            found_path = self.find_single_model_in_search_paths(model_name)
    ...
    def get_model_path(self, model_name: str, specific_model: str = "all") -> Optional[Path]:
        ...
        if model_info["type"] == "zip":
            ...
            if specific_model in found_models:
                return found_models[specific_model]
            elif "all" in found_models:
                # 指定されたモデルが見つからない場合は "all" を返す
                return found_models["all"]
            else:
                return None
        elif model_info["type"] == "runtime_download":
            # NudeNet models are handled by the package itself
            ...
            # Single file model - 柔軟検索を使用
            return self.find_single_model_in_search_paths(model_name)
