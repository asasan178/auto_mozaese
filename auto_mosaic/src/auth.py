...
# 開発者モード制御のための安全なprint関数
def auth_print(message, force_output=False):
    """認証システム用の開発者モード制御print関数"""
    if force_output:
        print(message)
        return
    ...
    try:
        ...
        # is_developer_mode関数が利用できない場合はコンソール出力なし
        pass
...
class MonthlyAuth:
    ...
    def _load_auth_config(self):
        """認証設定を読み込み（暗号化ファイル優先、次に環境変数、最後にデモ用）"""
        # 1. 暗号化設定ファイルから読み込みを試行
        if self._load_encrypted_auth_config():
            return
        ...
        if ENV_CONFIG_AVAILABLE:
            try:
                ...
                # 環境変数から月次パスワードとマスターパスワードを読み込み
                self.monthly_passwords = env_config.get_monthly_passwords()
                self.master_password_hash = env_config.get_master_password_hash()
                
                self._log_debug("Authentication config loaded from environment variables")
                
                # 設定の検証
                validation_results = env_config.validate_env_file()
                if validation_results:
                    for key, message in validation_results.items():
                        self._log_debug(f"Config validation - {key}: {message}")
                
                return
                
            except Exception as e:
                self._log_debug(f"Failed to load environment config: {e}")
        else:
            self._log_debug("Environment config not available")
            
        # 3. フォールバック: デモ用設定
        self._load_fallback_auth_config()
    
    def _load_encrypted_auth_config(self) -> bool:
        """暗号化設定ファイルから認証情報を読み込み"""
        try:
            from auto_mosaic.src.encrypted_config import EncryptedConfigManager
            
            manager = EncryptedConfigManager()
            decrypted_config = manager.decrypt_config()
            
            if decrypted_config and isinstance(decrypted_config, dict):
                # マスターパスワードハッシュを読み込み
                if 'master_password_hash' in decrypted_config:
                    self.master_password_hash = decrypted_config['master_password_hash']
                    self._log_debug("Master password hash loaded from encrypted config")
                else:
                    self.master_password_hash = hashlib.sha256("demo".encode()).hexdigest()
                    self._log_debug("Master password hash not found in encrypted config, using demo")
                
                # 月次パスワードを読み込み
                if 'monthly_passwords' in decrypted_config:
                    self.monthly_passwords = decrypted_config['monthly_passwords']
                    self._log_debug(f"Monthly passwords loaded from encrypted config: {len(self.monthly_passwords)} entries")
                else:
                    # 月次パスワードがない場合はデモ用を生成
                    ...
                    for month in range(1, 13):
                        ...
                        self.monthly_passwords[month_key] = hashlib.sha256(demo_password.encode()).hexdigest()
                    self._log_debug("Monthly passwords not found in encrypted config, using demo passwords")
                
                self._log_debug("Authentication config loaded from encrypted file")
                return True
            
        except Exception as e:
            self._log_debug(f"Failed to load encrypted auth config: {e}")
        
        return False
    
    def _load_fallback_auth_config(self):
        """フォールバック用のデモ認証設定を読み込み"""
        self._log_debug("Loading fallback demo authentication config")
        
        # デモ用月次パスワード（"demo_YYYY_MM"のハッシュ）
        self.monthly_passwords = {}
        for month in range(1, 13):
            month_key = f"2025-{month:02d}"
            demo_password = f"demo_{month_key.replace('-', '_')}"
    ...
    def get_online_time(self) -> tuple:
        ...
        # 開発モードでは簡易的な時刻同期のみ実行（デバッグ目的）
        try:
            from auto_mosaic.src.auth_config import AuthConfig
            auth_config = AuthConfig()
            is_dev_mode = auth_config.is_developer_mode()
            if is_dev_mode:
                self._log_debug("Development mode - using lightweight time sync for debugging")
        except Exception as e:
            # フォールバック: デフォルトは一般ユーザーモード
            self._log_debug(f"Failed to load AuthConfig in time sync: {e}")
            is_dev_mode = False
    ...
    def validate_system_time(self) -> tuple:
        ...
        if not sync_success:
            # オフライン環境での対応（開発モードは緩和、本番環境は制限）
            try:
                from auto_mosaic.src.auth_config import AuthConfig
                auth_config = AuthConfig()
                is_dev_mode = auth_config.is_developer_mode()
            except Exception as e:
                self._log_debug(f"Failed to load AuthConfig in offline handling: {e}")
                is_dev_mode = False  # デフォルトは一般ユーザーモード
            
            if is_dev_mode:
                self._log_debug("Development mode + offline - allowing with warning")
                return True, "開発モード（オフライン、時刻同期失敗）"
            else:
        ...
        try:
            ...
            self._log_debug(f"Failed to load AuthConfig in time diff check: {e}")
            is_dev_mode = False  # デフォルトは一般ユーザーモード
        
        if is_dev_mode:
            # 開発モード: 1時間まで許可（デバッグ効率重視）
            max_time_diff = 3600  # 1時間
            threshold_name = "開発モード"
        else:
    ...
    def show_auth_dialog(self, parent=None) -> bool:
        ...
        if not time_valid:
            ...
            if parent:
                ...
                auth_print(f"Authentication failed: {error_message}")  # エラーメッセージも開発者モード時のみ表示
            return False
        ...
        dialog.title("自動モザエセ v1.0 - 認証 (exe化対応)")
        ...
        try:
            ...
            auth_config = AuthConfig()
            is_dev_mode = auth_config.is_developer_mode()
            env_status = "開発モード" if is_dev_mode else ("EXE版" if self.is_exe else "本番版")
        except:
...
def authenticate_user(parent=None, force_auth_dialog=False) -> bool:
    ...
    try:
        ...
        try:
            ...
            auth_config = AuthConfig()
            is_dev_mode = auth_config.is_developer_mode()
            env_name = "Development" if is_dev_mode else ("EXE" if auth.is_exe else "Production")
        except Exception as e:
            auth._log_debug(f"Failed to load AuthConfig: {e}")
            is_dev_mode = False  # デフォルトは一般ユーザーモード
            env_name = "EXE" if auth.is_exe else "Production"
        
        auth._log_debug(f"Execution environment: {env_name}")
        auth._log_debug(f"Auth file path: {auth.auth_file}")
        auth._log_debug(f"Force auth dialog: {force_auth_dialog}")
        
        # 開発モードでの強制認証ダイアログ機能
        if force_auth_dialog and is_dev_mode:
            auth._log_debug("Development mode - forcing authentication dialog for debugging")
            auth_print("🔧 開発モード: 認証ダイアログを強制表示（デバッグ用）")
            ...
            auth_print("✅ 今月は既に認証済みです。認証をスキップします。")
            
            # 開発モードでは追加情報を表示
            if is_dev_mode:
                auth_print("💡 開発者向け: 強制認証テストには force_auth_dialog=True を使用してください")
        ...
        auth_print("🔐 月次認証が必要です。認証ダイアログを表示します。")
        ...
        if not result and parent:
            ...
            try:
                ...
                try:
                    ...
                    auth_config = AuthConfig()
                    is_dev_mode = auth_config.is_developer_mode()
                    env_status = "開発モード" if is_dev_mode else ("EXE版" if auth.is_exe else "本番版")
                except:
                    env_status = "EXE版" if auth.is_exe else "本番版"
                ...
                auth_print(f"[DEBUG] メッセージボックス表示エラー: {e}")
        
        return result
        
    except Exception as e:
        error_msg = f"認証システムエラー: {str(e)}"
        auth_print(f"[ERROR] {error_msg}")  # エラーメッセージも開発者モード時のみ表示
        ...
        try:
            ...
            auth_print(f"[DEBUG] Failed to load AuthConfig in error handling: {e}")
            is_dev_mode = False  # デフォルトは一般ユーザーモード
        
        if is_dev_mode:
            import traceback
            auth_print(f"[DEBUG] 詳細エラー情報:\n{traceback.format_exc()}")
        
        if parent:
            if is_dev_mode:
                # 開発モードでは詳細なエラー情報とデバッグオプションを提示
                from tkinter import messagebox
...
def clear_authentication_and_test(parent=None) -> bool:
    ...
    try:
        ...
        auth_print("🔧 認証状態をクリアしました。認証ダイアログを表示します。")
        return authenticate_user(parent, force_auth_dialog=True)
    except Exception as e:
        auth_print(f"❌ エラー: {e}")  # エラーメッセージも開発者モード時のみ表示
        return False
...
def generate_monthly_passwords():
    ...
    auth_print("月次パスワード一覧:")
    for month in ["2025-01", "2025-02", "2025-03", "2025-04", "2025-05", "2025-06",
                  "2025-07", "2025-08", "2025-09", "2025-10", "2025-11", "2025-12"]:
        password = generate_password()
        password_hash = auth.hash_password(password)
        auth_print(f"{month}: {password} -> {password_hash}")

def debug_authentication():
    """開発者向け認証デバッグ関数"""
    auth_print("🔧 認証システム デバッグモード")  # デバッグ関数も開発者モード時のみ表示
    auth_print("=" * 50)
    ...
    try:
        ...
        env_status = "EXE版" if auth.is_exe else "開発版"
    
    auth_print(f"実行環境: {env_status}")
    auth_print(f"認証ファイル: {auth.auth_file}")
    auth_print(f"認証ファイル存在: {auth.auth_file.exists()}")
    auth_print(f"今月認証済み: {auth.is_already_authenticated_this_month()}")
    
    # 現在の月のパスワードを表示（開発用）
    current_month = auth.get_current_month_key()
    auth_print(f"現在の月: {current_month}")
    
    # デバッグオプション
    auth_print("\nデバッグオプション:")
    auth_print("1. 通常認証テスト")
    auth_print("2. 強制認証ダイアログ表示")
    auth_print("3. 認証状態クリア")
    auth_print("4. 時刻検証テスト")
    auth_print("5. 月次パスワード確認")
    auth_print("0. 終了")
    ...
    try:
        ...
        if choice == "1":
            ...
            auth_print(f"結果: {'成功' if result else '失敗'}")
            ...
            auth_print("\n🔧 認証状態クリア実行中...")
            auth.clear_authentication_state()
            auth_print("認証状態をクリアしました")
            
        elif choice == "4":
            auth_print("\n🔧 時刻検証テスト実行中...")
            is_valid, message = auth.validate_system_time()
            auth_print(f"時刻検証結果: {'OK' if is_valid else 'NG'}")
            auth_print(f"メッセージ: {message}")
            
        elif choice == "5":
            auth_print("\n🔧 月次パスワード確認:")
            auth_print(f"現在の月: {current_month}")
            if current_month in auth.monthly_passwords:
                auth_print(f"パスワードハッシュ: {auth.monthly_passwords[current_month]}")
                auth_print("マスターパスワード: yofumoza")
            else:
                auth_print("❌ 現在の月のパスワードが設定されていません")
                
        elif choice == "0":
            auth_print("デバッグモード終了")
            return
